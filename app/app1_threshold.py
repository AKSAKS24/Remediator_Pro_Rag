...
class RulesetStore:
    ...
    # --- NEW: returns both text and score ---
    def get_relevant_rules_with_scores(self, user_query: str, top_k: Optional[int] = None):
        vectordb = self.load_or_reload()
        k = top_k if top_k is not None else self.top_k
        docs_and_scores = vectordb.similarity_search_with_score(user_query, k=k)
        return [(doc.page_content, score) for doc, score in docs_and_scores]

# Initialize RAG vector DB store (on startup)
ruleset_store = RulesetStore(RULESET_PATH, CHROMA_DIR, top_k=4)
...
# === MAIN REMEDIATION FUNCTION, with no-force-fallback logic ===
async def remediate_with_rag(unit: Unit) -> str:
    ...
    k = 4  # Top-N rules to fetch (tunable)
    rule_texts_and_scores = await asyncio.to_thread(
        ruleset_store.get_relevant_rules_with_scores, search_query, k
    )

    SIMILARITY_THRESHOLD = 0.4  # Tune as required
    selected_rules = []
    for i, (rule_text, score) in enumerate(rule_texts_and_scores):
        print(f"\n--- Candidate Rule {i+1}, Score: {score:.4f} ---\n{rule_text[:200]}...")
        if score <= SIMILARITY_THRESHOLD:
            selected_rules.append(rule_text)

    if not selected_rules:
        print("[INFO] No rule met the threshold; proceeding with empty rule context.")

    print(f"[RAG] {len(selected_rules)} rule(s) meet the threshold {SIMILARITY_THRESHOLD}")

    rules_text = "\n\n".join(selected_rules)  # empty string if no rule matched
